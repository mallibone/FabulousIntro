    ...
    let view (model: Model) dispatch =
        View.ContentPage(
          ...
          content = View.StackLayout(
            ...
            children = [ 
                //View.Entry(placeholder = "Username", 
                //    text = model.Username, 
                //    textChanged=(debounce 250 (fun e ->  dispatch (UsernameChanged e.NewTextValue))), 
                //    horizontalOptions = LayoutOptions.Center, widthRequest=300.0)
                makeEntry "Username" model.Username (fun e -> dispatch (UsernameChanged e.NewTextValue)) false
                View.Entry(placeholder = "Password", 
                    text = model.Password, 
                    textChanged=(debounce 250 (fun e ->  dispatch (PasswordChanged e.NewTextValue))), 
                    isPassword = true, 
                    horizontalOptions = LayoutOptions.Center, 
                    widthRequest=300.0)
